    ...
    <div class="card shadow-sm p-4 mb-4 d-flex flex-column justify-content-between"
        ...
        <form method="POST" action="{{ route('contenus.store') }}" enctype="multipart/form-data">
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="statut" value="{{ old('statut') }}">
                    @error('statut')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <input type="datetime-local" class="form-control" name="dateCreation" value="{{ old('dateCreation') }}">
                    @error('dateCreation')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idTypeContenu" required>
                        ...
                        @foreach ($typesContenu as $type)
                            <option value="{{ $type->id }}" {{ old('idTypeContenu') == $type->id ? 'selected' : '' }}>
                                {{ $type->nom }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control" name="idParent" value="{{ old('idParent') }}">
                    @error('idParent')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idAuteur" required>
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('idAuteur') == $user->id ? 'selected' : '' }}>
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idModerateur">
                        <option value="">Sélectionner...</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('idModerateur') == $user->id ? 'selected' : '' }}>
                                {{ $user->nom }} {{ $user->prenom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idRegion" required>
                        ...
                        @foreach ($regions as $region)
                            <option value="{{ $region->id }}" {{ old('idRegion') == $region->id ? 'selected' : '' }}>
                                {{ $region->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idLangue" required>
                        ...
                        @foreach ($langues as $langue)
                            <option value="{{ $langue->id }}" {{ old('idLangue') == $langue->id ? 'selected' : '' }}>
                                {{ $langue->nom }}
                            </option>
                        @endforeach
                    </select>
