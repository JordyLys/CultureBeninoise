    ...
    <div class="card shadow-sm p-4 mb-4 d-flex flex-column justify-content-between"
        ...
        <form method="POST" action="{{ route('commentaires.store') }}">
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" name="texte" required>{{ old('texte') }}</textarea>
                    @error('texte')
            ...
            <div class="row mb-5">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control" name="note" min="0" max="5" value="{{ old('note') }}" required>
                    @error('note')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="idContenu" required>
                        ...
                        @foreach ($contenus as $contenu)
                            <option value="{{ $contenu->id }}" {{ old('idContenu') == $contenu->id ? 'selected' : '' }}>
                                {{ $contenu->titre }}
                            </option>
                        @endforeach
                    </select>
